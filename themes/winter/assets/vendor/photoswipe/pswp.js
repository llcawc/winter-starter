// ------- Photoswipe: плагин просмотра изобржений -------------------

// ------- Функция парсинга кода галереи изображений для построения массива слайдов из списка ссылок---------------------------------
//
...
// Конец... initPhotoSwipeFromDOM(). Функция парсинга изображений

// Вставка кода photoswipe на страницу <div id="codpswp"></div>
// Эта функция загрузки html на страницу вызова галереи

var openPswp = function () {
  let codPswp = `
  ...
  // проверка пустой ли тег div с id=codpswp - обязательно в коде html проверять - тег должен быть пуст!
  let elCodPswp = document.getElementById("codpswp");
  if (elCodPswp) {
    let cont = elCodPswp.innerHTML.trim();
    if (cont.length == 0) {
      elCodPswp.insertAdjacentHTML("afterbegin", codPswp);
    }
  } else
    console.log(
      '!!! Не загружена разметка и не найден класс pswp! Добавь в body пустой div с id="codpswp"'
    );
};
// ------- Конец блока кода загрузки html на страницу вызова галереи -----------

// ------- Запуск функции загрузки кода разметки -------------------------------
openPswp();
...
// просто добавь класс '.gallery' в обёртку или одинокий 'figure'
...
// ------- End. Photoswipe: код плагина просмотра изобржений -------------------
